...
import { SizeTypesFormDialog } from "@/components/settings/SizeTypesFormDialog";
...
/** Плитка «Розміри» — заголовок відкриває попап, той самий патерн, що ColorsTile. Раніше — частина комбінованої плитки «Розміри та заміри», розділено за прямою вказівкою людини. */
export function SizesTile({
  sizeTypes,
  flags,
}: {
  ...
  const visible = sizeTypes.slice(0, MAX_VISIBLE_VALUES);
  const hasMore = sizeTypes.length > MAX_VISIBLE_VALUES;
  ...
  return (
    <div className="flex flex-col gap-3 rounded-xl bg-card p-4 ring-1 ring-foreground/10 transition-colors hover:ring-foreground/20">
        ...
        trigger={
          <button type="button" className="flex cursor-pointer items-start gap-3 text-left">
            ...
            <span className="flex min-w-0 flex-1 flex-col">
              <span className="truncate text-sm font-medium text-foreground">Розміри</span>
              <span className="truncate text-xs text-muted-foreground">Типи розмірів товару</span>
            </span>
            <span className="shrink-0 text-sm font-medium text-muted-foreground">{sizeTypes.length}</span>
      ...
      <ReferenceDictionaryFlagsRow dictionaryKey="sizes" flags={flags} />
    </div>
  );
}
