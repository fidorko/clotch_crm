...
| EditableSelectRow | ui/editable-select-row.tsx | той самий патерн «перегляд + олівчик → редагування», що й `PurchasePriceRow` (`ProductInfoPanel`), але для випадаючого списку: до кліку — просто текст, після кліку на олівчик — `Select` одразу відкритий (`defaultOpen`), закриття без вибору (Escape/клік повз) повертає в перегляд без зміни. 7 використань в `ProductInfoPanel` (Колекція, Стать, Сезон, Посадка, Виробник, Матеріал, Тип тканини) |
