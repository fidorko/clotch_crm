...
### care_instructions
Тенант-конфігурований довідник рекомендацій по догляду (назва + іконка): `tenant_id`, `name` (напр. «Ручне прання»), `icon` (text-ключ із фіксованого набору-кандидатів `CARE_INSTRUCTION_ICON_OPTIONS`, `lib/constants/care-instruction-icons.ts` — 12 lucide-іконок), `position`. UNIQUE `(tenant_id, name)`, індекс `(tenant_id, position)`. **Про запас, поки без споживача в UI**: спочатку планувався для «Рекомендацій по догляду» на типі тканини, але людина усвідомила помилку моделювання (нижче, `decisions.md`) — реальний догляд залежить від фактичного складу конкретного товару, а не від назви структури плетіння тканини, тож прив'язку до `fabric_types` прибрано. Таблиця/дані/дії створення (`createCareInstructionAction` тощо) лишені в коді недоторканими — за прямою вказівкою людини, «лишити про запас» для майбутньої картки товару. Дані-шар (`server/data/care-instructions.ts`) і Server Actions далі повністю робочі, просто без виклику з UI.
