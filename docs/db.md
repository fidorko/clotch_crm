...
### reference_items
Один спільний довідник для 8 «просто назва» пунктів «Довідники» (settings): `collections`/`seasons`/`fabric-materials`/`manufacturers`/`brands`/`countries`/`units`/`fit` — замість окремої таблиці на кожен тип. `tenant_id`, `kind` (enum `reference_item_kind`, той самий список значень, що `REFERENCE_ITEM_KINDS` у `lib/constants/reference-item-kinds.ts` — тримати синхронно вручну, schema-файли навмисно не імпортують з `lib/`), `name`, `position`. UNIQUE `(tenant_id, kind, name)` — дублікат назви в межах одного kind заборонено, той самий кортеж не заважає різним kind мати однакову назву. Індекс `(tenant_id, kind, position)`. Дев-сід заповнює 100 брендів (`lib/mocks/brands.ts`, kind=`brands`).
