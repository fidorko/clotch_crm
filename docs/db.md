...
Замість FK на 9 різних таблиць — той самий принцип, що `reference_dictionary_flags`: **довільний стабільний текстовий ключ** `characteristic_key` — `"colors"` / `"fabric-materials"` / `"care-instructions"` / `"custom:<custom_characteristics.id>"` / `"size-type:<size_types.id>"` / `"measurement-type:<measurement_types.id>"` / `"reference-item:manufacturers"` / `"reference-item:countries"` / `"suppliers"`.
...
Доступно лише для існуючих категорій (`/settings/categories/[id]`, не `/new` — потрібен `category.id`). Наступна задача (не зроблена зараз, узгоджено заздалегідь): сторінка товару після вибору категорії має динамічно показувати лише закріплені характеристики.
