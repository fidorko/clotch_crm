...
Таблиці з байтами: `product_photos.data`/`mime_type`, `product_color_photos.data`/`mime_type` (обидві вище), `category_images` (нова, нижче). Валідація файлу (розмір ≤ 5 МБ, MIME-білий список `image/jpeg`/`image/png`/`image/webp`) — спільна функція `validateImageFile` (`src/server/images/validate-image.ts`), викликається перед вставкою байтів у БД.

Роздача — через route handlers `src/app/api/uploads/{categories,products,product-colors}/[id]/route.ts`: `id` у URL — це `id` рядка таблиці (не `tenant_id`, не шлях на диску), маршрут сам бере `tenantId` із `getDevTenantId()` (сервер) і читає рядок саме цього тенанта (`readCategoryImage`/аналоги в `product-photos.ts`/`product-color-photos.ts`, фільтр `tenant_id` + `id`). Чужий тенант, знаючи чужий `id`, отримає 404 (рядок під його `tenant_id` просто не знайдеться) — той самий принцип ізоляції, що й скрізь у БД, через RLS + явний фільтр, а не через файлову систему. Відповідь — байти напряму (`new Response(new Uint8Array(data), { headers: { "Content-Type": mimeType, "Cache-Control": "private, max-age=3600" } })`).
