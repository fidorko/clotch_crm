...
UI — одна плитка `SizesMeasurementsTile`, попап `SizesMeasurementsFormDialog` із двома вкладками (`Tabs`, «Розміри»/«Заміри»), у кожній — спільний компонент `TypeValuesEditor` (картка на тип: назва інлайн-редагована, значення — чипи з input+Enter, як у `CustomCharacteristicFormDialog`), різні Server Actions на вкладку.
...
**Картка товару (`ProductSizeChart.tsx`) і далі читає власний хардкод** — підключення сторінки товару до цих довідників не робили в цьому раунді (не просили), `modules/settings.md` «Відкрито».
