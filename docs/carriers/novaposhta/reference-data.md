...
```
Не використовується в коді ще (`getWarehouses` не фільтрується за типом на цьому проході) — записано про запас, для майбутнього фільтра «показати лише поштомати».
