...
| src/server/warehouse/bin-label-pdf.ts | `buildBinLabelsPdf` — PDF `pdf-lib`, сторінка 101×101мм, 5 рядків рівномірно на всю висоту (`ROW_HEIGHT = 101мм/5`, розділені лінією); у рядку — назва зліва (жирний `Inter-Bold`, кегль = висота штрихкоду) + справжній Code128 справа (`bwip-js/node`, `scaleToFit` під залишок ширини після тексту) |
| src/server/warehouse/fonts/Inter-Bold.woff2 | шрифт для PDF — той самий Inter, що в UI, з кирилицею (стандартні 14 шрифтів `pdf-lib` кодують лише WinAnsi/латиницю); вбудовується напряму через `@pdf-lib/fontkit`, файл-актив у репозиторії, не npm-пакет (`typeface-inter` важить 15МБ усіх нарізок) |
...
## Зв'язки
Залежить від: `reference_items` (довідник «Країни»), `currencies` (довідник валют), `PhoneInput`/`Checkbox`/`RadioGroup`/`Switch`/`Dialog`/`Table`/`EditableTextRow` (ui-kit), `bwip-js`/`pdf-lib`/`@pdf-lib/fontkit` (нові залежності, погоджено — `decisions.md`)
Від нього залежать: — (склад/комірка ще ніде не використовуються як зв'язок)

## Зроблено
- 2026-08-04 — фікс сайдбару: на `/settings/warehouses/new`/`/settings/warehouses/[id]` ліве меню помилково підсвічувало «Категорії товару» замість «Склади» (`isTabActive` дефолтив на `"categories"`, бо ці сторінки — власний маршрут, не `?tab=`) — помітила людина, виправлено в `layout/Sidebar.tsx` (`ui-kit.md`).
- 2026-08-04 — назва на етикетці: жирний шрифт (`Inter-Bold.woff2`, замінив regular — на етикетці більше нічого текстового не малюється), кегль = висота штрихкоду (`boldFont.sizeAtHeight(BARCODE_HEIGHT)`) — за прямою вказівкою людини (перша спроба «максимально великий під висоту рядка» виявилась завеликою, людина поправила).
- 2026-08-04 — фікс: друк PDF падав (`WinAnsi cannot encode "А"`) — назви вулиць/стелажів/комірок людина вводить кирилицею, а стандартні 14 шрифтів `pdf-lib` кодують лише WinAnsi (латиницю). Вбудовано Inter (той самий шрифт, що в UI) напряму через `@pdf-lib/fontkit` (нова залежність) — файл шрифту як актив у репозиторії (`src/server/warehouse/fonts/`), не через важкий npm-пакет (`typeface-inter` важить 15МБ усіх нарізок, тут потрібен один файл). Заразом макет етикетки — за проханням людини: назва зліва, штрихкод справа на всю ширину 101мм (замість «текст над штрихкодом»); 5 рядків рівномірно розподілені по всій висоті сторінки (`ROW_HEIGHT = 101мм / 5`, без порожнього відступу знизу).
...
## Відкрито
- [ ] Штрихкод на етикетці масштабується `scaleToFit(ширина після тексту, BARCODE_HEIGHT)` — для довгих кодів, коли тексту лишає мало ширини, штрихкод може стискатись менше цільової висоти. Пробував масштаб лише за висотою (однаковий розмір завжди) — людина попросила повернути як було, не чіпати без окремого запиту
