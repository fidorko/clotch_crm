...
**Друк ЕН/маркування (`NovaPoshtaProvider.printDocuments`, ✅ реалізовано третім проходом)** — URL-схема `my.novaposhta.ua/orders/{method}/orders[]/{ref}/type/pdf/apiKey/{key}`, без мережевого виклику, підтверджена незалежним відкритим SDK (офіційний портал 403 для бота), `docs/carriers/novaposhta/printing.md`. Кнопки — у `SuccessPanel`, активні лише після успішного створення ЕН у тому самому замовленні.
...
- [ ] URL-схема друку (`my.novaposhta.ua/orders/...`) підтверджена незалежним відкритим SDK, не офіційним порталом напряму — перший живий друк вартий перевірки з людиною
