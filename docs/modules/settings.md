...
**Оновлено:** 2026-08-03 (розділ «Склади» винесено в піддокумент `settings-warehouses.md` одразу — той самий обсяг, що «Постачальники»)

## Призначення
Сторінка налаштувань магазину: розділи (вітрина, загальні, категорії товару, замовлення, довідники, склади, доставка, оплата, тарифний план) — вибираються через підменю сайдбару (розгортається інлайн під «Налаштування»), розділ у URL як `?tab=`. «Категорії товару» — повний CRUD з реальною БД (список/створення/редагування/видалення, ієрархія, зображення, успадкування від батьківської — `settings-category-characteristics.md`). «Довідники» — файли/дані/історія повністю в `settings-references.md` (модуль переріс ліміт розділу 0). «Склади» — повний CRUD (список+форма), файли/дані/історія в `settings-warehouses.md`.
...
## Відкрито
- [ ] 6 із 9 розділів без вмісту (Вітрина магазину, Загальні, Замовлення, Доставка, Оплата, Тарифний план) — плейсхолдер-текст «ще в розробці». «Склади» тепер на реальній БД — `settings-warehouses.md`
