...
**Оновлено:** 2026-08-05 (розділ «Доставка» винесено в піддокумент `settings-delivery.md` одразу — той самий обсяг, що «Склади»)

## Призначення
Сторінка налаштувань магазину: розділи (вітрина, загальні, категорії товару, замовлення, довідники, склади, доставка, оплата, тарифний план) — вибираються через підменю сайдбару (розгортається інлайн під «Налаштування»), розділ у URL як `?tab=`. «Категорії товару» — повний CRUD з реальною БД (список/створення/редагування/видалення, ієрархія, зображення, успадкування від батьківської — `settings-category-characteristics.md`). «Довідники» — файли/дані/історія повністю в `settings-references.md` (модуль переріс ліміт розділу 0). «Склади» — повний CRUD (список+форма), файли/дані/історія в `settings-warehouses.md`. «Доставка» — список способів доставки (CRUD), файли/дані/історія в `settings-delivery.md`.
...
## Відкрито
- [ ] 5 із 9 розділів без вмісту (Вітрина магазину, Загальні, Замовлення, Оплата, Тарифний план) — плейсхолдер-текст «ще в розробці». «Склади»/«Доставка» тепер на реальній БД — `settings-warehouses.md`/`settings-delivery.md`
