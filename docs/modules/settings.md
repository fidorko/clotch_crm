...
**Оновлено:** 2026-08-06 (новий розділ «Загальні» винесено в піддокумент `settings-general.md` одразу)

## Призначення
Сторінка налаштувань магазину: розділи (вітрина, загальні, категорії товару, замовлення, довідники, склади, доставка, оплата, тарифний план) — вибираються через підменю сайдбару (розгортається інлайн під «Налаштування»), розділ у URL як `?tab=`. «Категорії товару» — повний CRUD з реальною БД (список/створення/редагування/видалення, ієрархія, зображення, успадкування від батьківської — `settings-category-characteristics.md`). «Довідники» — файли/дані/історія повністю в `settings-references.md` (модуль переріс ліміт розділу 0). «Склади» — повний CRUD (список+форма), файли/дані/історія в `settings-warehouses.md`. «Доставка» — список способів доставки (CRUD), файли/дані/історія в `settings-delivery.md`. «Оплата» — список способів оплати (CRUD, 4 системні + власні), файли/дані/історія в `settings-payment.md`. «Загальні» — основні дані магазину (singleton) + список ФОП/ТОВ тенанта, файли/дані/історія в `settings-general.md`.
...
| — (див. `settings-general.md`) | Розділ «Загальні» (`?tab=general`) — «Основні дані» (`general_settings`, singleton, автозбереження) + «Мої ФОП та ТОВ» (`company_legal_entities`, CRUD). Файли/дані/історія — в піддокументі |
...
## Зроблено
- 2026-08-06 — новий розділ «Загальні» (`?tab=general`, за скріном-зразком людини): перша плитка 70% ширини «Основні дані» (назва/сайт/email/контактна особа+посада+телефон/графіки роботи — `WarehouseWorkHoursField` перевикористано напряму, ui-kit.md), singleton-рядок на тенанта (`general_settings`, `tenant_id` сам PK), автозбереження debounce (той самий принцип, що `WarehouseForm`). Друга плитка «Мої ФОП та ТОВ» (`company_legal_entities`) — тип (ФОП/ТОВ, кольорова іконка-абревіатура)/назва/ЄДРПОУ/статус, попап create+edit, клік по рядку відкриває той самий попап (повних банківських реквізитів не запитувалось — відкритий пункт). Деталі, файли, історія — в новому піддокументі `settings-general.md`
...
## Відкрито
- [ ] 3 із 9 розділів без вмісту (Вітрина магазину, Замовлення, Тарифний план) — плейсхолдер-текст «ще в розробці». «Склади»/«Доставка»/«Оплата»/«Загальні» тепер на реальній БД — `settings-warehouses.md`/`settings-delivery.md`/`settings-payment.md`/`settings-general.md`
